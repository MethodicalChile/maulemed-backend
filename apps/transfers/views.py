...
from apps.audit.services import audit_action, serialize_instance
...
STOCK_TRANSFER_ITEMS_QS = StockTransferItem.objects.select_related(
    "product",
    "product__category",
    "product__unit",
    "lot",
    "lot__warehouse",
    "lot__warehouse__branch",
    "lot__product",
...
class StockTransferViewSet(BaseModelViewSet):
    queryset = (
        StockTransfer.objects.select_related(
            ...
            "received_by",
            "parent_transfer",
            "parent_transfer__origin_branch",
            "parent_transfer__destination_branch",
        )
        .prefetch_related(
            Prefetch(
                "items",
                queryset=STOCK_TRANSFER_ITEMS_QS,
            )
        )
        .all()
    )
    ...
    def _reload_for_response(self, instance):
        """
        Recarga el traspaso con todas las relaciones del queryset optimizado.

        Los servicios de dominio pueden devolver una instancia actualizada,
        pero esa instancia no necesariamente conserva el prefetch de items.
        """
    ...
    def approve(self, request, uuid=None):
        ...
        stock_transfer = self._reload_for_response(
            stock_transfer
        )
    ...
    def reject(self, request, uuid=None):
        ...
        instance.rejection_reason = request.data.get(
            "rejection_reason"
        )
    ...
    def send(self, request, uuid=None):
        ...
        audit_action(
            ...
            old_data=old_data,
            new_data={
                "stock_transfer_uuid": str(
                    result["stock_transfer"].uuid
                ),
        ...
        return api_response(
            data={
                "stock_transfer": self.get_serializer(
                    stock_transfer
                ).data,
                "processed_items": result[
                    "processed_items"
...
class StockTransferItemViewSet(BaseModelViewSet):
    ...
    def get_queryset(self):
        ...
        return apply_branch_scope(
            qs,
            self.request.user,
            branch_field="stock_transfer__origin_branch",
        )
