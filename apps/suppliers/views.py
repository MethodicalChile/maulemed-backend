...
class SupplierViewSet(BaseModelViewSet):
    ...
    filterset_fields = (
        ["is_active", "allows_credit"]
        if hasattr(Supplier, "allows_credit")
        else ["is_active"]
    )
...
class SupplierProductViewSet(BaseModelViewSet):
    ...
    def product_price_history(
        ...
        """
        Historial de precios del producto agrupado por proveedor.

        El historial se precarga ya ordenado mediante Prefetch + to_attr,
        evitando ejecutar una consulta adicional por cada SupplierProduct.
        """
        try:
            product = (
        ...
        ordered_history_qs = (
            SupplierProductPriceHistory.objects.only(
                "id",
                "supplier_product_id",
                "effective_date",
                "price",
        ...
        supplier_products = (
            ...
            .prefetch_related(
                Prefetch(
                    "price_history",
                    queryset=ordered_history_qs,
                    to_attr="ordered_price_history",
                )
            )
            .order_by(
                "supplier__name",
        ...
        for supplier_product in supplier_products:
            points = [
                ...
                for price_record in supplier_product.ordered_price_history
            ]

            if (
                not points
                and supplier_product.last_price is not None
            ):
                points.append(
                    {
...
class SupplierProductPriceViewSet(BaseModelViewSet):
    queryset = SupplierProductPrice.objects.select_related(
        "supplier_product",
...
class SupplierProductPriceHistoryViewSet(BaseModelViewSet):
    queryset = (
        SupplierProductPriceHistory.objects.select_related(
            "supplier_product",
            "supplier_product__supplier",
            "supplier_product__product",
            "supplier_product__product__category",
